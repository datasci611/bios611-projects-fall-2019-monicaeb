...
p_length <- ggplot(data=dat.stat[dat.stat$diffdays<=179,]) +
  ...
  labs(title="Figure 5: Days spent at UMD Shelter (up to 95% quantile) by Housing Status",y="Days",x="Housing\nStatus")
p_length

ggsave(plot=p_length,filename="./report/p_length.png",device="png")
...
p_statdest <- ggplot(data=dat.dest, aes(status,fill=dest)) + 
  ...
  labs(title="Figure 6: Destination after UMD by Housing Status at Entry",x="Housing status",y="% of Clients") +
