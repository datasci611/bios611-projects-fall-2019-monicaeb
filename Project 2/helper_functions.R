...
# text to accompany box plot or bar plot for coming/going 

bptext_fun <- function(yrlow,yrhi) {
  
  if (yrlow==yrhi) {
    t <- ""
  } else {
    t <- "Blue points correspond to the mean for each month, horizontal lines are the medians, and purple points are outliers. Click on any point to see the number of people represented by it."
  }
